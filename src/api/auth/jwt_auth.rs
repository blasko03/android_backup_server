...
use actix_web::{Error, HttpMessage, HttpResponse, Responder, post, web};
...
pub async fn jwt_auth_middleware(
    ...
    let token = match req
        .headers()
        .get("Authorization")
        .ok_or("Authorization header missing".to_string())
        .and_then(|h| {
            h.to_str()
                .map_err(|_| "Authorization header invalid".to_string())
        })
        .and_then(|auth| {
            auth.strip_prefix("Bearer ")
                .ok_or("Authorization header invalid".to_string())
        })
        .and_then(|token| {
...
async fn login(auth: web::Json<JwtAuthJson>) -> impl Responder {
    ...
    match devices.get(auth.uuid.as_str()).map(|device_auth| {
        hex::decode(device_auth.password_hash.clone()).unwrap()
            == Sha256::digest(auth.password.clone()).as_slice()
    }) {
        Some(true) => {}
        _ => {
